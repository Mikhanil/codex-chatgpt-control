...
function detectExperienceFromSnapshot(snapshot) {
  ...
  if (/\/work(?:\/|$|\?)/.test(url)) {
    evidence.push({ source: "url", label: snapshot.url ?? CHATGPT_HOME2 });
  }
...
async function readSurfaceSnapshot(page) {
  ...
  const snapshot = await page.evaluate((surfaceOptionLabels) => {
    ...
    const composerLabels = Array.from(new Set(visibleComposerNodes.map(labelFor).map(normalize).filter(Boolean))).slice(0, 16);
